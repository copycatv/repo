...
void showTeach() {
	...
	// Add your code here
	snprintf(query, 1000,"select course.course_ID,course_name, offering_no, classroom, no_of_stds from course, \
			offering where offering.course_id = course.course_id and offering.staff_id = %d", staff_id);
...
void showTAs() {
	...
	snprintf(query, 1000, "select TA.student_ID, TA.first_name, TA.last_name, TA.phone from \
		TA, pref_ta where pref_ta.student_id = TA.student_id and pref_ta.staff_id = %d", staff_id);
	ret = SQLExecDirectA(hstmt, (SQLCHAR*)query, SQL_NTS);
	if (ret == SQL_SUCCESS || ret == SQL_SUCCESS_WITH_INFO) {
		printIntoRow(hstmt, 20);
	}
...
void showPre() {
	...
	snprintf(query, 1000, "select main_course_id, LISTAGG(prereq_course_id, ',') \
...
void showSuper() {
	...
	snprintf(query, 1000, "select TA.student_ID, first_name || ',' || last_name as FULLNAME from TA, \
		supervise where supervise.student_id = TA.student_id and supervise.staff_id = %d", staff_id);
	ret = SQLExecDirectA(hstmt, (SQLCHAR*)query, SQL_NTS);
	if (ret == SQL_SUCCESS || ret == SQL_SUCCESS_WITH_INFO) {
		printIntoRow(hstmt, 20);
	}
...
void showSuperGroup() {
	...
	snprintf(query, 1000, "select prof.staff_ID, prof.first_name || ' ' || prof.lastname as STAFF_FULL_NAME, \
		LISTAGG(TA.student_id || ':' || TA.first_name || '' || TA.last_name, ',') as STUDENTS \
		FROM TA, SUPERVISE, PROF where TA.student_id = SUPERVISE.staff_id GROUP BY (prof.staff_id, prof.first_name, prof.last_name)");
	ret = SQLExecDirectA(hstmt, (SQLCHAR*)query, SQL_NTS);
	if (ret == SQL_SUCCESS || ret == SQL_SUCCESS_WITH_INFO) {
		printIntoRow(hstmt, 20);
	}
...
void addPhone() {
	...
	cin >> newPhone;
	// TODO 6:  add a new phone number for the prof. assume user always enter a new phone number so you do not need to check
	// for the expect behaviour of this part please refer to the executable program
	// Add your code here

	snprintf(query, 1000, "INSERT INTO prof_phone values(%d,%d)", staff_id, newPhone);
	ret = SQLExecDirectA(hstmt, (SQLCHAR*)query, SQL_NTS);
	if (ret == SQL_SUCCESS || ret == SQL_SUCCESS_WITH_INFO) {
		cout << "SUCCESS in adding phone number!";
	}
	else{
		cout << "FAIL to add phone number!";
	}
	SQLFreeStmt(hstmt, SQL_CLOSE);
	system("pause");
